...
import io.github.effiban.scala2java.core.enrichers.contexts.matchers.TemplateBodyEnrichmentContextMockitoMatcher.eqTemplateBodyEnrichmentContext
...
class TemplateBodyEnricherImplTest extends UnitTestSuite {
  ...
  private val TheContext = TemplateBodyEnrichmentContext(JavaScope.Class, Some(t"MyClass"))
  ...
  test("enrich") {
    doAnswer((stat: Stat, _: TemplateBodyEnrichmentContext) => stat match {
    ...
    }).when(templateStatEnricher).enrich(any[Stat], eqTemplateBodyEnrichmentContext(TheContext))
