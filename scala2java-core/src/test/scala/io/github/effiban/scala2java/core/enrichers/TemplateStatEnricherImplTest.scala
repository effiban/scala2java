...
class TemplateStatEnricherImplTest extends UnitTestSuite {
  ...
  private val TemplateContextWithClassName = TemplateEnrichmentContext(JavaScope.Class, Some(ClassName))
  ...
  test("enrich() for secondary ctor. when class name provided") {
    ...
    val actualEnrichedCtorSecondary = templateStatEnricher.enrich(stat = SecondaryCtor, context = TemplateContextWithClassName)
  ...
  test("enrich() for secondary ctor. without class name should throw exception") {
    intercept[IllegalStateException] {
      templateStatEnricher.enrich(stat = SecondaryCtor, context = TemplateEnrichmentContext(javaScope = JavaScope.Class))
    }
  }
  ...
  test("enrich() for Defn.Var which is not an enum constant list") {
    ...
    val actualEnrichedDefnVar = templateStatEnricher.enrich(
      stat = TheDefnVar,
      context = TemplateEnrichmentContext(javaScope = JavaScope.Class)
    )
