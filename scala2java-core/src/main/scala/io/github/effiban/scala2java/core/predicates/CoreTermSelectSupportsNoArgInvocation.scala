...
private[predicates] class CoreTermSelectSupportsNoArgInvocation(termSelectClassifier: TermSelectClassifier,
  ...
  private def supportsByQualifiedName(termSelect: Term.Select) = termSelect match {
      ...
      case Term.Select(qual: Term.Select, q"apply") if termSelectClassifier.hasApplyMethod(qual) => true
      case Term.Select(qual: Term.Select, q"empty") if termSelectClassifier.hasEmptyMethod(qual) => true
