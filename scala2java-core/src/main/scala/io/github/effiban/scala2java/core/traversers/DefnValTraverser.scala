...
import io.github.effiban.scala2java.core.traversers.results.DefnValTraversalResult

import scala.meta.Defn

trait DefnValTraverser {
  def traverse(defnVal: Defn.Val, context: StatContext = StatContext()): DefnValTraversalResult
}
...
private[traversers] class DefnValTraverserImpl(modListTraverser: => ModListTraverser,
                                               ...
                                               expressionTermTraverser: => ExpressionTermTraverser) extends DefnValTraverser {

  //TODO if it is non-public it will be invalid in a Java interface - replace with method
  override def traverse(valDef: Defn.Val, context: StatContext = StatContext()): DefnValTraversalResult = {
