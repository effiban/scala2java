...
@deprecated
private[traversers] class DeprecatedIfTraverserImpl(expressionTermTraverser: => DeprecatedExpressionTermTraverser,
                                                    blockTraverser: => DeprecatedBlockTraverser)
                                                   (implicit javaWriter: JavaWriter) extends DeprecatedIfTraverser {
  ...
  override def traverse(`if`: If, shouldReturnValue: Decision = No): Unit = {
    ...
    write("if (")
    expressionTermTraverser.traverse(`if`.cond)
    write(")")
  ...
  override def traverseAsTertiaryOp(`if`: If): Unit = {
    ...
    expressionTermTraverser.traverse(`if`.cond)
    write(") ? ")
    expressionTermTraverser.traverse(`if`.thenp)
    write(" : ")
    `if`.elsep match {
