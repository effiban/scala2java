...
trait TemplateStatEnricher {
  def enrich(stat: Stat, context: TemplateStatEnrichmentContext = TemplateStatEnrichmentContext()): EnrichedStat
}
...
private[enrichers] class TemplateStatEnricherImpl(ctorSecondaryEnricher: CtorSecondaryEnricher,
  ...
  override def enrich(stat: Stat, context: TemplateStatEnrichmentContext = TemplateStatEnrichmentContext()): EnrichedStat = stat match {
  ...
  private def enrichSecondaryCtor(secondaryCtor: Ctor.Secondary, context: TemplateStatEnrichmentContext) = {
  ...
  private def enrichRegularStat(stat: Stat, context: TemplateStatEnrichmentContext) = {
    defaultStatEnricher.enrich(stat, StatContext(context.javaScope))
  }

  private def toCtorContext(childContext: TemplateStatEnrichmentContext, className: Type.Name) = {
    CtorSecondaryEnrichmentContext(javaScope = childContext.javaScope, className = className)
  }
}
