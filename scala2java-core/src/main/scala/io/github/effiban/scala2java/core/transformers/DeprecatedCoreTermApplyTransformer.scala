...
// TODO remove once the core Term.Name-s are fully qualified at start of flow
@deprecated
private[transformers] class DeprecatedCoreTermApplyTransformer(termNameClassifier: TermNameClassifier,
  ...
  def transformOptional(termApply: Term.Apply, context: TermApplyTransformationContext) = {
    termApply match {
