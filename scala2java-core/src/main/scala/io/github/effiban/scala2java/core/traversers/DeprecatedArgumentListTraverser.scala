...
@deprecated
trait DeprecatedArgumentListTraverser {
  def traverse[T <: Tree](args: List[T],
                          argTraverser: => DeprecatedArgumentTraverser[T],
                          context: ArgumentListContext = ArgumentListContext()): Unit
}

@deprecated
class DeprecatedArgumentListTraverserImpl(implicit javaWriter: JavaWriter) extends DeprecatedArgumentListTraverser {
