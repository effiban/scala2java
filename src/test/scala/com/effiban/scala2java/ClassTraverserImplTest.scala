...
class ClassTraverserImplTest extends UnitTestSuite {
  ...
  test("traverse() for case class with one list of ctor args and no body") {
    ...
    verifyModifiersResolverInvocationForCaseClass()
  }

  test("traverse() for case class with two lists of ctor args and no body") {
    val modifiers: List[Mod] = List(
      Mod.Annot(
        Init(tpe = Type.Name(AnnotationName), name = Name.Anonymous(), argss = List())
      ),
  ...
  test("traverse() for regular class") {
    val modifiers: List[Mod] = List(
      Mod.Annot(
        Init(tpe = Type.Name(AnnotationName), name = Name.Anonymous(), argss = List())
      )
    ...
    verifyModifiersResolverInvocationForRegularClass()
  }
  ...
  private def verifyModifiersResolverInvocationForCaseClass() = {
  ...
  private def verifyModifiersResolverInvocationForRegularClass() = {
    verify(javaModifiersResolver).resolveForClass(modsCaptor.capture)
    val actualMods = modsCaptor.value
