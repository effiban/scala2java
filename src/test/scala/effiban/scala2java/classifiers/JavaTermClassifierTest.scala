...
import effiban.scala2java.classifiers.JavaTermClassifier.requiresStatementEnd
...
import scala.meta.{Case, Init, Lit, Name, Pat, Template, Term, Type}

class JavaTermClassifierTest extends UnitTestSuite {
  ...
  private val RequiresStatementEndScenarios = Table(
  ...
  forAll(RequiresStatementEndScenarios) { case (desc: String, term: Term, expectedResult: Boolean) =>
    test(s"requiresStatementEnd($desc) should be $expectedResult") {
      requiresStatementEnd(term) shouldBe expectedResult
    }
  }
}
